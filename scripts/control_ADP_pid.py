...
from ADP.adp import PIDSMADP
from ADP.adp import PSMADP
...
if __name__ == "__main__":
	...
	while not rospy.is_shutdown():
		...
		if agent.global_flag == 1:  # approaching
			...
			uav_ros = UAV_ROS(m=0.72, g=9.8, kt=1e-3, dt=agent.DT, time_max=20)  # 0.722
			...
			dualpid_ctrl = PidControl(kp_pos=np.array([4.5, 4.5, 5.]),
                     ki_pos=np.array([0.01, 0.01, 0.01]),
                     kd_pos=np.array([3.4, 3.2, 3.]),
                     kp_att=np.array([6.5, 6.5, 2.5]),
                 	 ki_att=np.array([0.02, 0.02, 0.02]),
                	 kd_att=np.array([0.0, 0.0, 0.]),
                 	 p_v=np.array([0.6, 0.6, 0.8]),
                     p_r=np.array([0.75, 0.75, 0.75]))
			...
			if agent.controller == 'PX4-PID':
				...
				adp_x.update_sm(state=uav_state[0: 6])
    			adp_x.update_reward(state=uav_state[0: 6], ref_state=uav_ref_state[0:6])
       
       			sx_ = adp_x.s 
       			rx = adp_x.r
				adp_x.trainI(sx, sx_, rx, )
				...
				phi_d, theta_d, uf = uo_2_ref_angle_throttle(dualpid_ctrl.control - np.array([0.1*observe[0], 0.1*observe[1], 1*observe[2]]),
															 uav_ros.uav_att(),
															 psi_d,
															 uav_ros.m,
															 uav_ros.g,
